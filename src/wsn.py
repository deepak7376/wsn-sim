...
class WSN:
    ...
    def _create_edgelist_from_csv(self, path):
        ...
        for i, elrow in edgelist.iterrows():
            if (
                elrow[3] == True
            ):  # if node 1 and node 2 are neighbor then there is edge between them
    ...
    def _create_coordinate(self):
        ...
        coords = [
            {
                "id": id,
                "X": random.randint(0, x_coord),
                "Y": random.randint(0, y_coord),
                "data": 0,
                "power": self.config.get_remaining_power_of_node,
                "status": True,
            }
    ...
    def _calculate_eculidean_dist(self, node_1, node_2):
        return round(
            ((node_1["X"] - node_2["X"]) ** 2 + (node_1["Y"] - node_2["Y"]) ** 2)
            ** 0.5,
            2,
        )
    ...
    def _visualize_graph(self):
        ...
        node_positions = {
            node[0]: (node[1]["attr_dict"]["X"], node[1]["attr_dict"]["Y"])
